...
Asena.addCommand({pattern: 'search', fromMe: false, desc: FIND_DESC }, (async (message, match) => {
    if (message.reply_message === false) return await message.client.sendMessage(message.jid, '*අනිවාර්යයෙන් ඔබ mp3 එකකට reply දැමිය යුතුයි x-devil!🌠*', MessageType.text);
    ...
    ffmpeg(filePath).format('mp3').save('music.mp3').on('end', async () => {
        ...
        await axios.post('https://api.audd.io/', form, configs).then(async (response) => {
            ...
            if (res === 'success') {
                ...
                await message.client.sendMessage(message.jid, '*කණගාටුයි එය හමුනොවීය..', MessageType.text);
            }
        }).catch((error) =>  {
            console.log(error);
        });
